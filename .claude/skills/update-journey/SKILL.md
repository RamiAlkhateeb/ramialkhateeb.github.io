---
name: update-journey
description: Update the career journey section (map + always-visible timeline). Use when the user asks to add, edit, or sync a job/role/experience entry, or to update the résumé-driven timeline.
---

# Update the career journey

The `#journey` section of the single page (`index.html`) shows career history in two parts, both driven by `window.RAMI_JOURNEY` in `js/journey.js` (kept in sync with the résumé at `assets/senior_software_engineer.md`, the source of truth for dates, titles, and achievements):
...
```js
{
  ...
  x: 26, y: 15,               // % position (0-100) matching that country's pixel location on assets/images/Europe-middle-east.png (see below)
...
- **`x`/`y` must be identical for every entry sharing the same `country`** (all UAE entries today use `x:85,y:69`).

## The map background

`renderJourneyMap()` overlays markers and an animated route (SVG `viewBox="0 0 100 100"`) on top of the real map image `assets/images/Europe-middle-east.png` (1050×760px, despite the `.png` extension it's actually a GIF — browsers render it fine via content sniffing, but be aware if a build tool ever inspects the file). A marker's `x`/`y` (see `js/journey.js`) is that country's position as a **percentage of the image's pixel width/height**: `x = pixelX/1050*100`, `y = pixelY/760*100`. Since the container (`.journey-sketch`) is locked to the image's exact aspect ratio (`aspect-ratio:1050/760` in `css/site.css`) and the image uses `object-fit:cover`, percentage positions stay aligned with the image at any screen size. Current coordinates are an eyeballed approximation (documented in a comment at the top of `js/journey.js`) — nudge them by eye against the actual image if they're off. The source image is a light-toned map; `.journey-map-img` in `css/site.css` inverts it (`filter:invert(1) hue-rotate(180deg) …`) so it reads as a night chart against the dark theme with the orange route glowing over it — if the image is ever replaced with an already-dark one, drop that filter. The map is **not mirrored in RTL** (unlike the earlier abstract-sketch version) since it depicts real geography — flipping a real map would make it geographically backwards.
